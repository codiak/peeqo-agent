...
async function pickFile(folderPath){
	...
	if(mediaFiles.length === 0){
		debug(`No media files found in ${folderPath}`)
		return null
	}
...
async function transitionFromMedia(ms, interruptible = false){
	...
	return new Promise((resolve) => {
		...
		function done() {
			if (interruptible) removeListeners()
		...
		function removeListeners() {
			event.removeListener('stop-media', onStop)
			event.removeListener('pause-media', onPause)
			event.removeListener('resume-media', onResume)
		}

		if (interruptible) {
			event.on('stop-media', onStop)
			event.on('pause-media', onPause)
			event.on('resume-media', onResume)
		}
	})
}
